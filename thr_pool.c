...
int main (int argc,char * argv[])
{
	...
	pool = thr_pool_create(num_thread, num_thread, 0, 0);
	...
	//avail = 0;
	sum = malloc(sizeof(int)* 4);
	for(i = 0; i < 4;i++){
		sum[i] = 0;
		avail[i] = 1;
	}
	td = malloc(sizeof(struct thread_data*) * 3200);
